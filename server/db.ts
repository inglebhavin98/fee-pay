...
export async function initializeDatabase() {
  ...
  try {
    ...
    const testStudents = [
      ...
      { email: 'alice@somemail.com', name: 'Alice Smith', class: 5, section: 'B' },
      { email: 'bob@somemail.com', name: 'Bob Johnson', class: 6, section: 'A' },
      { email: 'carol@somemail.com', name: 'Carol Williams', class: 6, section: 'B' }
    ];
    ...
    for (const student of testStudents) {
      ...
      if (studentExists.rows.length === 0) {
        ...
        // Create a single fee entry for testing
        console.log(`Creating fee entry for ${student.name}...`);
        await pool.query(
          "INSERT INTO fees (student_id, type, amount, due_date, status) VALUES ($1, $2, $3, $4, $5)",
          [result.rows[0].id, 'Annual Fee', 5000, new Date('2024-03-31'), 'UNPAID']
        );
      }
    }
